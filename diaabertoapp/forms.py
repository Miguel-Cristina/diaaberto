...
from .models import Atividade, Campus, Edificio, Sala, Tematica, Tarefa, PublicoAlvo, Faculdade, Departamento, MaterialQuantidade, Material, Sessao, SessaoAtividade, TipoAtividade
import datetime
...
class CampusForm(forms.ModelForm):
    nome = forms.CharField(label="", widget=forms.TextInput(attrs ={'class':"input",'type':"text"}),error_messages={'unique': 'Um campus com o mesmo nome já existe! Por favor coloque outro campus.'})
    class Meta:
        model = Campus
        fields = ('nome',)
...
class EdificioForm(forms.ModelForm):
    ...
    nome = forms.CharField(label="", widget=forms.TextInput(attrs ={'class':"input",'type':"text"}),error_messages={'unique_together':'Um edificio com esse nome já existe!', 'required':'Preencha este campo.','unique': 'Um edificio com o mesmo nome já existe! Por favor coloque outro edificio.'})
    class Meta:
        model = Edificio
        fields = ('campus','nome',)
        error_messages = {
...
class SalaForm(forms.ModelForm):
    ...
    identificacao = forms.CharField(label="", widget=forms.TextInput(attrs ={'class':"input",'type':"text"}),error_messages={'unique': 'Uma sala com o mesmo nome já existe! Por favor coloque outra sala.'})
...
class SessaoAtividadeForm(forms.ModelForm):
    dia = forms.DateField(required=False, label="", widget=forms.DateInput(attrs={'class': "input", 'placeholder': "Dia"}))
    sessao = forms.ModelChoiceField(required=False, queryset=Sessao.objects.all(), label="",widget=forms.Select(attrs={'class':"select is-fullwidth",'style':"width:100%"}))
...
class AtividadeForm(forms.ModelForm):
    ...
    faculdade = forms.ModelChoiceField(required=True, initial='2',queryset=Faculdade.objects.all(), label="",widget=ModelSelect2Widget(model=Faculdade,search_fields=['nome__icontains'],attrs={'class':"tabfields",'id':"meuDepartamento",'style':"width:100%",'data-minimum-input-length':"0",'data-placeholder':"Selecione a faculdade...",'oninput':"$(this).removeClass('invalid');"})) 
    departamento = forms.ModelChoiceField(required=True, initial='1', queryset=Departamento.objects.all(), label="",widget=ModelSelect2Widget(model=Departamento,search_fields=['nome__icontains'],dependent_fields={'faculdade':'faculdade'},max_results=50,attrs={'class':"tabfields",'id':"escolherDepartamento",'style':"width:100%",'data-minimum-input-length':"0",'data-placeholder':"Selecione o departamento..."})) #'disabled':"true",
    campus = forms.ModelChoiceField(required=False,queryset=Campus.objects.all(), label="",widget=ModelSelect2Widget(model=Campus,search_fields=['nome__icontains'],attrs={'class':"tabinputs",'style':"width:100%",'data-minimum-input-length':"0",'data-placeholder':"Selecione o campus..."}))
    edificio = forms.ModelChoiceField(required=False,queryset=Edificio.objects.all(), label="",widget=ModelSelect2Widget(model=Edificio,search_fields=['nome__icontains'],dependent_fields={'campus':'campus'},attrs={'class':"tabinputs",'style':"width:100%",'data-minimum-input-length':"0",'data-placeholder':"Selecione o edificio..."}))
    sala = forms.ModelChoiceField(required=False,queryset=Sala.objects.all(), label="",widget=ModelSelect2Widget(model=Sala, search_fields=['identificacao__icontains'],dependent_fields={'edificio':'edificio'},attrs={'class':"tabinputs",'style':"width:100%",'data-minimum-input-length':"0",'data-placeholder':"Selecione a sala..."}))
